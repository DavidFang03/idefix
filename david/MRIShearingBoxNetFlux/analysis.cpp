     ...
     **************************************************************** */
{
  ...
  MPI_Reduce(&SquaredIntegral, &reducedSquaredIntegral, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
     ...
     **************************************************************** */
{
  ...
  for(int k = d->beg[KDIR]; k < d->end[KDIR] ; k++) {
    for(int j = d->beg[JDIR]; j < d->end[JDIR] ; j++) {
      for(int i = d->beg[IDIR]; i < d->end[IDIR] ; i++) {
        outfield += 2*3.1415*abs(d->Vc(BX1,k,j,i))/(d->dx[IDIR][i])/sqrt(d->Vc(RHO,k,j,i));
      }
    }
  }
     ...
     **************************************************************** */
{
  ...
  for(int k = d->beg[KDIR]; k < d->end[KDIR] ; k++) {
    for(int j = d->beg[JDIR]; j < d->end[JDIR] ; j++) {
      for(int i = d->beg[IDIR]; i < d->end[IDIR] ; i++) {
        outfield += 2*3.1415*abs(d->Vc(BX2,k,j,i))/d->dx[JDIR][j]/sqrt(d->Vc(RHO,k,j,i));
      }
    }
  }
     ...
     **************************************************************** */
{
  ...
  for(int k = d->beg[KDIR]; k < d->end[KDIR] ; k++) {
    for(int j = d->beg[JDIR]; j < d->end[JDIR] ; j++) {
      for(int i = d->beg[IDIR]; i < d->end[IDIR] ; i++) {
        outfield += 2*3.1415*abs(d->Vc(BX3,k,j,i))/d->dx[KDIR][k]/sqrt(d->Vc(RHO,k,j,i));
      }
    }
  }
